...
#creates data frame presenting cumulative lag coefficient estimates for continuous and bins models
cumcoeff_summary <- bind_rows(
  hypotheses(cont_0001_asinh_model, cont_hyp_string("l", "asinh", "0001"), countyyear_cluster_vcov(cont_0001_asinh_model)) %>%
    summarise('Cumulative Lag Estimate' = first(estimate), 'Std. Error' = first(std.error)) %>% mutate(Model = "Continuous Exposure"),
  hypotheses(binary_model, bin_hyp_string("l", "binary", "0_10"), countyyear_cluster_vcov(binary_model)) %>%
    summarise('Cumulative Lag Estimate' = first(estimate), 'Std. Error' = first(std.error)) %>% mutate(Model = "Binary Bins Exposure", Bin = "Binary (0 to 10km)"),
  hypotheses(binary_model, bin_hyp_string("l", "binary", "10_50"), countyyear_cluster_vcov(binary_model)) %>%
    summarise('Cumulative Lag Estimate' = first(estimate), 'Std. Error' = first(std.error)) %>% mutate(Model = "Binary Bins Exposure", Bin = "Binary (10 to 50km)"),
  hypotheses(binary_model, bin_hyp_string("l", "binary", "50_100"), countyyear_cluster_vcov(binary_model)) %>%
    summarise('Cumulative Lag Estimate' = first(estimate), 'Std. Error' = first(std.error)) %>% mutate(Model = "Binary Bins Exposure", Bin = "Binary (50 to 100km)"),
  hypotheses(days_model, bin_hyp_string("l", "days", "0_10"), countyyear_cluster_vcov(days_model)) %>%
    summarise('Cumulative Lag Estimate' = first(estimate), 'Std. Error' = first(std.error)) %>% mutate(Model = "Days Bins Exposure", Bin = "Days (0 to 10km)"),
  hypotheses(days_model, bin_hyp_string("l", "days", "10_50"), countyyear_cluster_vcov(days_model)) %>%
    summarise('Cumulative Lag Estimate' = first(estimate), 'Std. Error' = first(std.error)) %>% mutate(Model = "Days Bins Exposure", Bin = "Days (10 to 50km)"),
  hypotheses(days_model, bin_hyp_string("l", "days", "50_100"), countyyear_cluster_vcov(days_model)) %>%
    summarise('Cumulative Lag Estimate' = first(estimate), 'Std. Error' = first(std.error)) %>% mutate(Model = "Days Bins Exposure", Bin = "Days (50 to 100km)")
) %>% select(Model, Bin, 'Cumulative Lag Estimate', 'Std. Error')
...
saveRDS(cumcoeff_summary, here("source_data", "cumcoeff_summary.rds"))
